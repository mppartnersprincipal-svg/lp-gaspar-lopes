...
| ~~`produto-calcas`~~ | ~~Calça cós duplo (verde)~~ **Removida em 13/09/2026** (cliente pediu para excluir a foto; o card 05 Calças da vitrine saiu junto, sem substituta) | `Fotos - Produtos/Calças/Calça cós duplo.jpg` (original mantido na pasta) |
...
## Política de privacidade (publicada 08/09/2026)
`politica-de-privacidade.html` na raiz, servida em `/politica-de-privacidade` (o `cleanUrls:true` do `vercel.json` cuida disso; o `dev-server.mjs` espelha esse comportamento). Descreve só o que o site realmente faz (tracker anônimo, GTM/GA4/Ads com Consent Mode, chaves `gl-consent`/`gl-s`/`gl-v`) e traz um botão que revoga o consentimento apagando `gl-consent` e `gl-v` do navegador. Linkada no rodapé e dentro do banner de cookies; está no `sitemap.xml`.
**Falta o cliente informar** (o `[PREENCHER]` segue comentado no HTML, nunca supor): razão social, CNPJ e e-mail do encarregado/DPO. Enquanto não vierem, o canal para exercer direitos é o WhatsApp, que é dado real.
